...
public actor AudioPreloadService {
    ...
    public func startPreload(for asset: AVAsset) async throws {
        ...
        if fastWindowDuration > 5.0 {
            ...
            logger.debug("路径 B: \(path_B_Start, privacy: .public)s–\(path_B_Start + path_B_Duration, privacy: .public)s（补充首屏）")
        ...
        if preloadDuration > 0 {
            logger.debug("路径 C: \(preloadStart, privacy: .public)s–\(preloadStart + preloadDuration, privacy: .public)s（后台预加载）")
    ...
    public func getFirstFrameBuffer() async throws -> AudioBuffer {
        let firstFrameRange = CMTimeRange(
            start: .zero,
            duration: CMTime(seconds: min(5.0, strategy.fastFirstFrameDuration), preferredTimescale: 600)
        )
    ...
    private func extractAndCache(
        ...
        do {
            let startTime = Date()
            logger.debug("[\(label, privacy: .public)] 开始抽取: \(start, privacy: .public)s–\(start + duration, privacy: .public)s")
            
            let buffer = try await extractor.extract(from: asset, timeRange: timeRange)
            
            let elapsed = Date().timeIntervalSince(startTime)
            logger.info("[\(label, privacy: .public)] 抽取完成: 耗时=\(elapsed * 1000, privacy: .public)ms, 样本数=\(buffer.samples.count, privacy: .public)")
            ...
            logger.error("[\(label, privacy: .public)] 抽取失败: \(error.localizedDescription, privacy: .public)")
        }
    }
