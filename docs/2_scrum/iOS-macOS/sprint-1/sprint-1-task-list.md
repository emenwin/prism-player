...
- **验收标准**:
  - [x] UTF-8 编码，无 BOM ✅
  - [x] 文件命名：`<源文件名>.<locale>.srt`（如 `video.zh-Hans.srt`）✅
  - [x] 时间戳格式正确（`HH:MM:SS,mmm --> HH:MM:SS,mmm`）✅
  - [x] 文件名冲突自动处理（追加 -1, -2 后缀）✅
  - [x] 空间检查与错误提示（磁盘不足警告）✅
  - [x] 专项导出用例：空字幕/特殊字符/长文本/边界条件 ✅
  - [ ] 导出成功率 ≥ 99%（E2E 自动化测试）⏳ PR3
...
- **技术要点**:
  ...
  - 导出进度回调（为 UI 准备）
- **实施计划**:
  - PR1: SRT 格式化核心实现（0.5d）✅ 2025-11-13 完成
  - PR2: 文件系统操作与错误处理（0.5d）✅ 2025-11-13 完成
  - PR3: 集成测试与 CI 配置（0.5d）⏳ 待实施
- **实施成果**:
  - PR1 & PR2 ✅（Commit eb7412e）
  - SRTExporter.swift（329 行）
  - SRTExporterTests.swift（356 行）
  - 25 个单元测试全部通过
  - SwiftLint 严格模式通过
- **测试计划**:
  - 25 个单元测试（格式、边界、错误）✅ 已完成
  - 3 个 E2E 测试（完整导出、性能、文件冲突）⏳ PR3
  - 测试夹具：sample-10-lines.json, sample-utf8.json, sample.srt（金样本）⏳ PR3
