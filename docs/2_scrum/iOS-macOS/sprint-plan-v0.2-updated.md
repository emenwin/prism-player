...
- Sprint 1（M1 原型 - ASR 核心，建议 2 周，~40 SP）
- **Sprint 1b（播放器基础，建议 2 周，~34 SP）** ⭐ 新增
...
**Sprint 1b 说明**：
- 时间：2025-11-14 ~ 2025-11-27（Sprint 1 之后，Sprint 2 之前插入）
- 目标：完成播放器基础播放功能（优先 macOS），用于后续的集成测试
- 范围：播放控件增强、SRT 字幕加载/显示、视频拖动处理
- 价值：为 Sprint 2 的 ASR 集成测试提供稳定的播放器基础
...
## 3.5 Sprint 1b（播放器基础，建议 2 周，~34 SP）⭐ 新增

- Sprint Goal：完成播放器基础播放功能（优先 macOS），用于后续的集成测试。
- 背景：Sprint 1 完成了 ASR 核心能力（音频抽取、识别、字幕生成、SRT 导出），但播放器功能相对简化。为了在 Sprint 2 进行完整的集成测试（增量识别 + 播放器联动），需要先完善播放器的基础能力。
- 范围与 PBIs
  1) 视频选择功能（macOS 优先）（5 SP，已在 Sprint 1 完成，标记为完成）
     - AC：NSOpenPanel/UIDocumentPickerViewController 选择；支持常见格式；错误处理。
  2) 播放控件增强（8 SP，PRD §6.1.2）
     - AC：
       - 播放/暂停按钮、进度条、时间显示（00:00 / 05:30）
       - 播放速度选择（0.5x–2.0x）
       - 音量控制（可选）、全屏切换（macOS）
       - 键盘快捷键（macOS）：Space、Left/Right、Up/Down
       - 错误处理：播放失败、解码错误
  3) SRT 字幕加载（5 SP，PRD §6.1.3）
     - AC：
       - SRTParser 实现：解析 SRT 格式、支持多种编码（UTF-8, GBK 等）、容错处理
       - SubtitleSegment 模型：id, startTime, endTime, text
       - 字幕文件选择（手动）+ 自动加载同名字幕
       - 字幕文件状态管理（未加载/加载中/已加载/失败）
       - 单元测试：标准格式、时间戳解析、多行文本、容错
  4) SRT 字幕显示（8 SP，PRD §6.5）
     - AC：
       - SubtitleRenderer 组件：根据播放时间显示对应字幕（±50ms）
       - 字幕样式：底部居中、半透明背景、动态字号
       - 字幕开关与状态持久化
       - 多行字幕支持、最大行数限制
       - 性能优化：二分查找、避免重渲染
       - macOS/iOS 适配、a11y 支持
  5) 视频拖动处理（Seek）（8 SP，PRD §6.1.2, §6.4）
     - AC：
       - 进度条拖动：平滑体验、实时预览时间、精确跳转（±100ms）
       - 播放器 Seek：AVPlayer.seek(to:)、跳转完成回调
       - 字幕同步更新：Seek 后立即显示目标时间字幕、快速定位
       - 性能优化：防抖处理、避免频繁 Seek
       - 边界处理：文件开始/结束、无效时间戳、Seek 失败
       - 播放状态保持：播放中/暂停状态一致
- 交付物：
  - macOS 完整播放器（包含所有控件）
  - iOS 基础播放器（可选，时间允许）
...
- DoD 强化：
  ...
  - SwiftLint 严格模式通过
- 风险与回退：
  - 跨平台 UI 适配复杂 → 优先 macOS，iOS 可简化或延后
  - SRT 编码兼容性 → 优先 UTF-8，其他编码可选
  - 字幕同步性能 → 优先正确性，性能优化可迭代
