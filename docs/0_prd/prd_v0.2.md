...
- 目标
  ...
  - 在用户选择时，将识别结果翻译为目标语言字幕（离线优先，至少支持英↔中），并与播放同步展示与导出。
- 用户价值
  - 学习外语/专业内容时快速理解要点。
  - 为创作者提供可编辑的草稿字幕基础。
...
- In Scope（首期）
  ...
  - 端侧 ASR 与字幕生成（带时间戳）。
  - 字幕翻译（目标语言可选，离线优先；首批支持英文→中文，后续扩展更多语对）。
  - 字幕展示与基础样式（字号、主题、背景）。
...
9) 作为用户，我可以为外语音频选择翻译字幕并在播放时看到翻译结果。
- AC1：在设置或播放页可选择“字幕语言”：原文/翻译（选择目标语言）/（可选）双语。
- AC2：翻译字幕遵循同样的时间同步原则（偏差 ≤ ±200ms），优先显示当前窗口翻译；未完成的片段显示“翻译中…”。
- AC3：离线优先（本地翻译模型）；当本地不支持所选语对时，可提示下载对应模型；（可选）用户显式同意后可使用联网文本翻译，仅上传转写文本，不上传音视频。
- AC4：导出可选择原文轨或翻译轨，文件命名携带目标语言标记。
...
#### 6.1.1 媒体文件选择
- macOS: 使用 NSOpenPanel 选择本地视频/音频文件
- iOS: 使用 UIDocumentPickerViewController 选择文件
- 支持常见格式：mp4, mov, m4v, avi, mkv, mp3, m4a, wav, aac, flac
- 文件路径安全访问与权限处理（Sandbox、Security-Scoped Bookmarks）
- 错误处理：文件不存在、权限拒绝、格式不支持、解码失败
...
- **播放控制**:
  ...
  - 停止播放
- **进度管理**:
  - 进度条（Slider）：显示当前播放进度、支持拖动跳转、显示缓冲进度
  - 时间显示：当前播放时间 / 总时长（格式：HH:MM:SS 或 MM:SS）
  - 进度拖动（Seek）：平滑拖动、实时预览时间、释放后精确跳转（±100ms）
- **播放速度**:
  - 支持 0.5x, 0.75x, 1.0x, 1.25x, 1.5x, 2.0x
  - 倍速变化不影响音调（pitch-preserved）
  - 倍速设置持久化
- **音量与显示**:
  - 音量控制滑块（可选）
  - 全屏/窗口化切换（macOS）
  - 画面比例适配（aspect fill/fit）
- **键盘快捷键（macOS）**:
  - Space: 播放/暂停
  - Left/Right Arrow: ±5s 跳转
  - Shift + Left/Right: ±10s 跳转
  - Up/Down Arrow: 音量调节 ±10%
  - F: 全屏切换
  - 0-9: 跳转到 0%-90% 位置
- **进度变化事件**: 对字幕渲染有确定行为（见 6.4）
- **错误处理**: 播放失败、解码错误、网络错误（如适用）、文件损坏

#### 6.1.3 SRT 字幕文件支持（外部字幕）
- **字幕文件加载**:
  - 支持选择外部 SRT 字幕文件（NSOpenPanel/UIDocumentPickerViewController）
  - 自动加载同名字幕：查找同目录下同名 .srt 文件（如：video.mp4 → video.srt）
  - 支持多种编码：UTF-8, UTF-8-BOM, GBK, GB2312, Big5 等
- **SRT 格式解析**:
  - 标准 SRT 格式支持（序号、时间戳、文本）
  - 时间戳格式：HH:MM:SS,mmm（如：00:01:23,456）
  - 多行文本支持
  - 特殊字符与 HTML 标签容错
  - 格式容错：允许时间戳格式变体、允许缺失序号
- **字幕状态管理**:
  - 未加载、加载中、已加载、解析失败
  - 字幕文件路径显示
  - 重新加载与切换字幕文件
- **错误处理**:
  - 文件不存在、解析失败、编码错误、时间戳错误
  - 用户友好的错误提示与恢复建议
...
#### 6.5.2 字幕时间同步（外部 SRT）
- **时间精度**: ±50ms（外部字幕回放，无需 ASR 级别 ±200ms）
- **同步机制**: 根据播放器当前时间显示对应字幕片段
- **进度拖动同步**: 
  - Seek 后立即显示目标时间字幕
  - 字幕片段快速定位（二分查找）
  - 无字幕时显示空白或占位
...
#### 6.5.3 性能优化
- 二分查找当前时间对应片段（O(log n)）
- 避免不必要的重渲染
- 平滑的进入/退出动画（可选，≤200ms）

#### 6.5.4 跨平台适配
- macOS: NSView/SwiftUI Text 实现
- iOS: UIView/SwiftUI Text 实现
- 响应式布局：横屏/竖屏适配

#### 6.5.5 可访问性（a11y）
- VoiceOver: 字幕内容朗读（可选开关）
- 动态字体: 支持系统字体大小设置
- 高对比度: 自动适配高对比度模式
...
- 模型：选择、下载/导入、删除、查看版本与大小。
 - 语言：ASR 语言自动/指定；字幕语言：原文/翻译（选择目标语言，如 zh-Hans、en-US 等）；记住最近一次选择。
- 预加载时长：10/30/60s。
...
### 6.10 字幕翻译（新增）
- 目标：将 ASR 原文字幕按用户选择的目标语言进行离线翻译，并与播放进度同步显示与导出。
- 路径：
  - 优先采用“ASR 转写 → 本地 NMT 翻译（Marian/NLLB 等轻量模型，或 MLX 上的量化小模型）”两段式流水线；
  - 当 ASR 引擎（如 Whisper）提供“直接翻译”模式且满足目标语言时，可直出翻译文本（需保证时间戳一致性）。
- 范围：首批语对英文→中文（en→zh-Hans），后续按模型可用性扩展；用户可回退为原文字幕。
- 调度：翻译任务与识别任务解耦，但同样遵循窗口优先级（抢占 > 当前窗口滚动 > 预加载）。
- 模型管理：翻译模型采用与 ASR 相同的下载/导入/删除/校验流程，展示体积与语言覆盖；当空间不足时提供清理建议。
- UI：
  - 播放页提供“字幕语言”选择；双语模式（可选）以两行呈现（上原文/下翻译），避免遮挡内容；
  - 未完成片段显示“翻译中…”占位；失败片段可重试。
- 导出：支持选择导出原文或翻译轨；文件命名为 <源文件名>.<目标语言>.srt；UTF-8。
- 隐私：默认离线；若启用联网翻译，需显式同意，且仅上传转写文本，不上传音视频；设置中可随时关闭。
...
- 隐私与合规
  - 默认离线工作；不上传音视频或字幕内容。翻译默认使用本地模型；若启用联网翻译，仅在用户明确同意后上传转写文本，且不持久化到服务端。
...
3.1) 翻译字幕（可选）
- 用户选择“字幕语言”为翻译（如 zh-Hans）→ 对应窗口的已出 ASR 片段进入翻译队列 → 翻译完成即刻渲染 → 未完成片段显示“翻译中…”。
...
- M2 可用版
  - 增量识别与滚动字幕、模型管理（下载/导入/删除）、语言选择、缓存管理、一键清理、倍速播放适配、翻译字幕基础版（英文→中文，离线优先）、VTT（可选）。
- M3 优化版
  - 性能与能耗优化、进度拖动抢占识别、翻译语对扩展与质量优化、可访问性完善、故障与错误码体系、质量感知与按段重识别、稳定性与长视频策略完善。
...
- 风险：
  ...
  - 翻译模型体积与质量 → 首批限定语对与小模型，提供回退到原文字幕；必要时提示联网翻译可用（需显式同意）。

## 13. 指标与本地分析（不出网）

- 关键指标：首帧时间、RTF、识别段耗时、识别失败率、翻译延迟（ASR→译文段的延迟 P95）、导出使用率、模型选择分布（含翻译模型）。
